...
data RegFile = RegFile { regs  :: Map Reg32 Val
                       , temps :: TempFile
                       }
...
regInsert (Fixed r@(Reg32 n)) v@(S _) rf
  | n == 6 || n == 7 = do
    let regs' = Map.insert r v (regs rf)
    return $ rf { regs = regs' }
regInsert (Fixed (Reg32 n)) _ _
  | n == 6 || n == 7 = fail "write of non-stack address to stack register"
regInsert (Fixed r) v rf = do
    let regs' = Map.insert r v (regs rf)
    return $ rf { regs = regs' }
regInsert (Flex t) v rf = do
    let temps' = Map.insert t v (temps rf)
    return $ rf { temps = temps' }

regDelete :: Reg -> RegFile -> RegFile
regDelete (Fixed r) rf = rf { regs = Map.delete r (regs rf) }
regDelete (Flex t)  rf = rf { temps = Map.delete t (temps rf) }
...
run :: Monad m => FrameMap -> Label -> m Output
run fs l = do
--   IBlockFrame _ _ blocks <- Map.lookup l fs
--   block <- Map.lookup l blocks
  let block = [CALL l]
  let (result, output) = runWriter $ runErrorT (exec block `evalStateT` initSt `runReaderT` (FlowChart fs l))
  case result of
    Left msg -> fail (foldl (++) "\n" output ++ msg)
    Right () -> return output
...
run fs l = do
  IBlockFrame _ _ blocks <- Map.lookup l fs
  block <- Map.lookup l blocks
  case runWriterT (exec block `evalStateT` initSt `runReaderT` (FlowChart fs l)) of
    Left msg -> fail msg
...
{-
setCurrFrame :: (MonadReader [IntelFrame], MonadReader IBlockMap) =>
  Label -> m a -> m a
setCurrFrame l cmd = dolocal (\ (fm, _) -> (fm, Map.))
 -}
